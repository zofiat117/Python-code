...
player_wages = df['wage_eur']

# Compute and display the mean
mean_value = round(statistics.mean(player_wages), 2)
print("Mean Value:", type(mean_value),mean_value)

# Compute and display the median
median_value = statistics.median(player_wages)
print("Median Value:", median_value)

# Compute and display the min and max values
print("Min: €%f, Max: €%f" %(min(player_wages),max(player_wages)))

# Second smallest unique value
s1 = list(set(sorted(player_wages)))
s2 = sorted(s1)
print("Min:", s2[1])
